...
MYSQL_RES * mysql_select_files(MYSQL* conn, struct tm *date_mx, int hit, int flag) {
	...
	if(!flag) {
		if( domain != "" ) {
			sprintf(q, "SELECT file, domain, filesize, downloaded FROM haarp WHERE domain = '%s' AND date(downloaded) >= '%s' AND date(downloaded) <= '%s' AND requested = %i limit %i", domain.c_str(), ds.c_str(), d.c_str(), hit, NUM_FILES);
		}
		else {
			sprintf(q, "SELECT file, domain, filesize, downloaded FROM haarp WHERE date(downloaded) >= '%s' AND  date(downloaded) <= '%s' AND requested = %i  limit %i", ds.c_str(), d.c_str(), hit, NUM_FILES);
		}
	}
	else {
		if( domain != "" ) {
...
void mysql_delete_files(MYSQL * conn, struct tm * date_mx, int hit, int flag) {
	...
	if( !flag ) {
		if( domain == "" )
			sprintf(q,"DELETE from haarp WHERE date(downloaded) >= '%s' AND date(downloaded) <= '%s' AND requested = %i limit %i", ds.c_str(), d.c_str(), hit, NUM_FILES);
		else 
			sprintf(q,"DELETE from haarp WHERE domain = '%s' AND date(downloaded) >= '%s' AND date(downloaded) <= '%s' AND requested=%i limit %i", (sqlconv2(domain)).c_str(), ds.c_str(), d.c_str(), hit, NUM_FILES);
	} else {
		if( domain == "" )  
			sprintf(q,"DELETE from haarp WHERE date(downloaded) >= '%s' AND date(downloaded) <= '%s' AND requested >= %i limit %i", ds.c_str(), d.c_str(), hit, NUM_FILES);
		else 
		...
		while ( (r = mysql_fetch_row(res)) != NULL ) {
			...
			for(dir_index = list_dir.begin();dir_index != list_dir.end();dir_index++) {
				...
				if(file_exists(string(f))) {
					...
					numdelete++;
					if(atof(r[2]) > 1048576)
						printf("(%i) Delete [%.1lf MB] %s\t\t\t (%s) (t:%.2lf GB)\n", numdelete, atof(r[2])/(1048576.0), f, r[3], *mb/1024);
					else
						printf("(%i) Delete [%.1lf KB] %s\t\t\t (%s) (t:%.2lf GB)\n", numdelete, atof(r[2])/(1024.0), f, r[3], *mb/1024);
					system(q);					
...
void phelp(int flag) {
	if(flag)
		puts(" haarpClean 1.1 by Manolo Canales <keikurono01>");
	puts("\n\tUsage: \n");
