        ...
        for(int i=0;i < (int)list_dir.size();i++) {
			...
			cout<<"Disk occupation percentage on partition hosting cache directory "<<list_dir[i]<<" : "<<use_percent<<"%"<<endl;
					if( use_percent >= atof(cachelimit.c_str()) ) {
						...
						printf("Disk space occupied over cache limit : %.2lf MB (%.2lf GB) %.2lf %% above limit in /etc/haarp/haarp.conf (%.2lf %%)\n", expected_deletion/1024, expected_deletion/1073741824, use_percent - atof(cachelimit.c_str()), cachelimit.c_str());
